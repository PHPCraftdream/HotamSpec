...
| `R-operator-prompt-from-substrate` | DRAFT | `ai-agent` | A-compaction-loses-working, A-bootstrap-self-applies | The operator-facing portion of `CLAUDE.md` shall be generated from the substrate's SETTLED requirements (not hand-written), so the operator is the rendered form of the spec, not its author. |
