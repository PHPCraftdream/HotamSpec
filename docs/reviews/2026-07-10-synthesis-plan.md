...
- Эскалация отложенных D3 (бинарный enforcement) и E1 (компактный индекс)? (L2-#8)
  → **РЕШЕНО (Этап U, #123): оба отклонены.** D3 — формальный REJECTED-узел
  `R-binary-enforcement-gradient`: ratchet-давление уже бинарно
  (`is_closeable_debt()` смотрит только на `enforceability`, игнорируя
  PROSE↔STRUCTURAL); бинаризация — ~50-точечный рефактор ради нулевого сдвига
  метрики долга (после Этапа T реальный closeable-долг = 2 узла). Трёхуровневый
  enforcement сохранён как ДЕСКРИПТИВНАЯ, не нормативная шкала. E1 — declined
  БЕЗ граф-узла (никогда не был формальным Requirement, только backlog-пункт):
  экономия ~2900 символов из бюджета 150000 при огромном headroom не стоит
  риска — имена ENFORCED-правил нужны агенту для цитирования на LOCATE/CONFRONT
