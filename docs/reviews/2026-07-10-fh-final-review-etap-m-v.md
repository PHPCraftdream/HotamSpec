...
OPEN→SETTLED для `R-decided-by-verifiable-signature` c `enforceability=INHERENTLY_PROSE`. Весь контекст исходного OPEN (DEL-1, six-lens Enf#2/Vision#1, A-single-human-wears-all-hats) сохранён внутри нового `why` вместе с явным revisit-триггером (второй живой steward). Дифф совпадает с сообщением; docs/gen (OPEN.md, UNENFORCED.md, REQUIREMENTS.md, check.md) согласованы.
