...
|---|---|---|---|---|
| `R-87` | SETTLED | `platform` | A-sync-budget | The system shall return a payment decision in < 200 ms (p95). |
| `R-203` | SETTLED | `finance` | A-sync-budget | The system shall run a full synchronous AML/compliance check before approving any payment. |
| `R-90` | SETTLED | `product` | A-single-customer | The system shall personalize the checkout per returning customer. |
| `R-150` | SETTLED | `product` | A-single-customer | The system shall retain full per-customer transaction history for analytics. |
| `R-205` | OPEN(which permission scopes may an admin assume?) | `product` | A-eu-only | The system shall let an account administrator act on behalf of other users in the same organization. |
| `R-300` | DRAFT | `architecture` | A-sync-budget | The system shall pre-screen returning customers asynchronously and fast-path the synchronous check when a fresh clear exists. |

## Stakeholders

| id | name | domain |
|---|---|---|
| `finance` | Finance | money / compliance |
| `platform` | Platform | latency / SLA |
| `product` | Product | customer experience |
| `security` | Security | privacy / access |
| `architecture` | Architecture | cross-cutting structure |

## Assumptions

| id | status | owner | statement |
|---|---|---|---|
| `A-single-customer` | DEAD | `product` | Each account belongs to exactly one customer (no orgs). |
