...
// TestSelectTier1_DomainDirIgnored_NoFilesystemTrustShift proves the
// resolver never trusts a domain-local test tree: a Requirement whose
// enforced_by names a Test* function that exists ONLY under a domain
...
func TestSelectTier1_DomainDirIgnored_NoFilesystemTrustShift(t *testing.T) {
	...
	testFile := "package gen\n\nimport \"testing\"\n\nfunc Test_R_should_not_resolve(t *testing.T) {\n\tif false {\n\t\tt.Fatal(\"unreachable\")\n\t}\n}\n"
	...
	if _, ok := names["Test_R_should_not_resolve"]; ok {
		t.Fatalf("expected TestFuncNames to NOT include a domain-local gen/go test function; the gen-code trust shift was removed")
	}

	g := &ontology.Graph{
		DomainDir: domainDir,
		Requirements: []ontology.Requirement{
			{ID: "R-should-not-resolve", EnforcedBy: []string{"Test_R_should_not_resolve"}},
		},
	}
	result := SelectTier1("R-should-not-resolve", g)
	if result.Confident {
		t.Fatalf("expected confident=false: enforced_by entry only resolves via a domain-local gen/go tree, which must no longer be trusted")
	}
}

// TestSelectTier1_EmptyDomainDir_RegressionSafe proves the no-domain-context
// path (g.DomainDir == "") still resolves against internal/+cmd/ exactly as
// before.
func TestSelectTier1_EmptyDomainDir_RegressionSafe(t *testing.T) {
	t.Parallel()
	g := &ontology.Graph{
		Requirements: []ontology.Requirement{
