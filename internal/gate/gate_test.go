...
// TestFuncNames_ScansDomainGenGoWhenPresent proves the task #214 widening: a
// consumer domain's own <domain>/gen/go/**/*_test.go tree is scanned when
// domainDir is passed, so a Test* function generated by `hotam gen-code` into
// a synthetic domain directory (built here as a scratch fixture, mirroring
// domains/<d>/gen/go/*_test.go — see docs/GEN-CODE-CONTRACT.md §1) resolves
// exactly like an internal/cmd test does. This is what makes ENFORCED with a
// real enforced_by test structurally reachable for a consumer domain, not
// just self-hosted domains (task #197 finding, task #214 fix).
func TestFuncNames_ScansDomainGenGoWhenPresent(t *testing.T) {
	t.Parallel()
	domainDir := t.TempDir()
	genGoDir := filepath.Join(domainDir, "gen", "go")
	if err := os.MkdirAll(genGoDir, 0o755); err != nil {
		t.Fatalf("MkdirAll gen/go: %v", err)
	}
	testFile := "package gen\n\nimport \"testing\"\n\nfunc Test_R_consumer_domain_probe(t *testing.T) {\n\tif 1+1 != 2 {\n\t\tt.Fatal(\"arithmetic broke\")\n\t}\n}\n"
	if err := os.WriteFile(filepath.Join(genGoDir, "requirements_test.go"), []byte(testFile), 0o644); err != nil {
		t.Fatalf("WriteFile fixture: %v", err)
	}

	names, err := TestFuncNames(domainDir)
	if err != nil {
		t.Fatalf("TestFuncNames(%q): %v", domainDir, err)
	}
	if _, ok := names["Test_R_consumer_domain_probe"]; !ok {
		t.Fatalf("expected TestFuncNames to include the domain's gen/go test function, got %v", names)
	}
	// The always-present internal/cmd roots must still be scanned alongside
	// the new gen/go root -- this is additive, not a replacement.
	if _, ok := names["TestCheckAxisInRegistry_OK"]; !ok {
		t.Fatalf("expected TestFuncNames to still include internal/ test names alongside the new gen/go root")
	}
}

// TestSelectTier1_ResolvesEnforcedByFromDomainGenGo is the end-to-end proof
// requested by task #214: a Requirement whose enforced_by names a Test*
// function that lives ONLY under a consumer domain's gen/go tree (not
// anywhere in HotamSpec's own internal/ or cmd/) now resolves confidently
// through SelectTier1 when g.DomainDir points at that domain, exactly the
// scenario check_enforced_by_resolvable must also accept.
func TestSelectTier1_ResolvesEnforcedByFromDomainGenGo(t *testing.T) {
	t.Parallel()
	domainDir := t.TempDir()
	genGoDir := filepath.Join(domainDir, "gen", "go")
	if err := os.MkdirAll(genGoDir, 0o755); err != nil {
	...
	testFile := "package gen\n\nimport \"testing\"\n\nfunc Test_R_brd_integrity_zero_blockers(t *testing.T) {\n\tif false {\n\t\tt.Fatal(\"unreachable\")\n\t}\n}\n"
	if err := os.WriteFile(filepath.Join(genGoDir, "requirements_test.go"), []byte(testFile), 0o644); err != nil {
		t.Fatalf("WriteFile fixture: %v", err)
	}
	...
	g := &ontology.Graph{
		DomainDir: domainDir,
		Requirements: []ontology.Requirement{
			{ID: "R-brd-integrity", EnforcedBy: []string{"Test_R_brd_integrity_zero_blockers"}},
		},
	}
	result := SelectTier1("R-brd-integrity", g)
	if !result.Confident {
		t.Fatalf("expected confident=true for an enforced_by entry resolvable via the domain's gen/go tree, got false: %s", result.Reason)
	}
	found := false
	for _, id := range result.NodeIDs {
		if id == "Test_R_brd_integrity_zero_blockers" {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected NodeIDs to include the gen/go-resolved test name, got %v", result.NodeIDs)
	}
}

// TestSelectTier1_EmptyDomainDir_NoGenGoScan_RegressionSafe proves the
// self-hosted / no-domain-context path is byte-for-byte unaffected: when
// g.DomainDir is "" (e.g. a graph built in-memory by a test fixture with no
// loader-populated DomainDir), genGoRoot must not fire and resolution must
// fall back to exactly the pre-task-#214 internal/+cmd/ scan.
func TestSelectTier1_EmptyDomainDir_NoGenGoScan_RegressionSafe(t *testing.T) {
	t.Parallel()
	g := &ontology.Graph{
		Requirements: []ontology.Requirement{
			{ID: "R-test-func-2", EnforcedBy: []string{"TestCheckAxisInRegistry_OK"}},
		},
	}
	if g.DomainDir != "" {
		t.Fatalf("test fixture precondition: expected empty DomainDir, got %q", g.DomainDir)
	}
	result := SelectTier1("R-test-func-2", g)
	if !result.Confident {
		t.Fatalf("expected confident=true (unaffected by the gen/go widening), got false: %s", result.Reason)
	}
}

// TestGenGoRoot_MissingDirIsNotAnError proves a domain that never ran
// `hotam gen-code` (no gen/go directory at all) is handled as "nothing to
// scan", not as an error -- genGoRoot's ok return is false, and callers
// (testFuncRoots) simply omit the root rather than failing the whole scan.
func TestGenGoRoot_MissingDirIsNotAnError(t *testing.T) {
	t.Parallel()
	domainDir := t.TempDir() // no gen/go subdirectory created
	if _, ok := genGoRoot(domainDir); ok {
		t.Fatalf("expected genGoRoot to report ok=false for a domain with no gen/go directory")
	}
	if _, ok := genGoRoot(""); ok {
		t.Fatalf("expected genGoRoot to report ok=false for an empty domainDir")
	}
}
