...
// (docs/GEN-CODE-CONTRACT.md §7, R-domain-overview-projection) that answers
...
// generated markdown under docs/gen/, not zero-Cyrillic gen-code; see
// docs/GEN-CODE-CONTRACT.md's zero-Cyrillic rule, which governs .go output
// only). A domain with zero Process nodes gets an honest, valid, non-empty
// placeholder — never a blank file or a generator error (§7's opt-in
// aspect: §Process is optional, so "no processes modeled yet" is a normal
// domain shape, not a defect).
func BuildPipeline(g *ontology.Graph, domainName string) string {
	...
	lines = append(lines, "")
	lines = append(lines, "Generated from the domain's own `Process` nodes (§Process, the opt-in behavioral aspect: a Lifecycle + ordered Steps + roles_required + drives_entities) and the EntityTypes those processes drive — a whole-domain narrative answering \"how does this work end to end?\" without reading the source prose (docs/GEN-CODE-CONTRACT.md §7, R-domain-overview-projection).")
	lines = append(lines, "")
	lines = append(lines, "---")
	lines = append(lines, "")
