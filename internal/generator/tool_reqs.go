...
var toolRequirementData = []ToolRequirement{
	...
	{Basename: "context_producer", CanonSection: "Context", Claim: "the producer half of the context cipher, writing spec/.runtime/context.json.", Enforcer: ""},
	{Basename: "create_agent", CanonSection: "Agent", Claim: "scaffolds spec/agents/<name>/ as a self-contained sub-operator directory with its own CLAUDE.md, scope.py, tools/, agents/, and README.md.", Enforcer: "test_tool_create_agent"},
	...
	{Basename: "create_domain", CanonSection: "Domain", Claim: "scaffolds domains/<name>/ as a self-contained business domain with manifest.py, graph.py, tools/, agents/director/, docs/gen/, and CLAUDE.md.", Enforcer: "test_tool_create_domain"},
	...
	{Basename: "gate_status", CanonSection: "Closure", Claim: "read spec/.runtime/land-log.jsonl and answer the commit-boundary question.", Enforcer: "test_tool_gate_status"},
	{Basename: "gen_spec", CanonSection: "Generator", Claim: "regenerates docs/gen/ from the executable model (docstrings + graph), making drift structurally impossible.", Enforcer: ""},
	...
	{Basename: "invoke_agent", CanonSection: "Agent", Claim: "invokes a sub-agent by loading its spec/agents/<name>/CLAUDE.md as the operator-prompt and printing it to stdout.", Enforcer: "test_tool_invoke_agent"},
	{Basename: "land", CanonSection: "Closure", Claim: "single CLI entry point over gate.py/gate_status.py/closure.py.", Enforcer: "test_tool_land"},
	...
	{Basename: "setup_context_hook", CanonSection: "Context", Claim: "installs/removes the project-local hook that feeds tools/context_producer.py.", Enforcer: "test_tool_setup_context_hook"},
	...
	{Basename: "spawn_log_isolation_status", CanonSection: "Agent", Claim: "reads spec/.runtime/spawn-log.jsonl and flags mutating agents recorded without worktree isolation.", Enforcer: "test_tool_spawn_log_isolation_status"},
...
func BuildToolDerivedSection() string {
	...
	lines = append(lines, "")
	lines = append(lines, "Projected from `spec/tools/*.py` module docstrings whose first line matches `Canon: §<topic> — <claim>` (R-tool-is-its-own-requirement). The docstring IS the claim; the body IS the check; the test IS the enforcer. Deleting the tool deletes the R.")
	lines = append(lines, "")
	if len(toolReqs) == 0 {
