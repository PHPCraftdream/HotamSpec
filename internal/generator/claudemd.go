...
// tools_data.go), sorted by Command, one line per tool with a Ported/Declared
// marker driven by the tool's Status field. It used to be the hand-maintained
// embeddedToolsText constant in claudemd_static.go; that constant silently
// drifted out of sync every time a Python-era tool got ported to a real Go
// command (P1-6 / TaskList #19 found it still calling `req`/`due`/`inspect`/
// `land` "not yet ported" after all four had working cmd/hotam commands).
// Deriving the block from the registry instead makes that class of drift
// structurally impossible, the same way gen-spec's whole existence does for
// docs/gen/*.md (R-... generative law: an important-yet-invisible fact goes
// into a typed node, not prose someone forgets to update).
func RenderEmbeddedToolsBlock() string {
	...
	for _, t := range sorted {
		...
		case methodology.Ported:
			statusSuffix = " Ported (`hotam " + displayName + "`)."
		default:
			// Declared tools' Purpose text already opens with "Not
			// ported. Historically: ..." (see tools_data.go) — appending
			// another "Not yet ported." would read as a stutter, so only
			// add the suffix when the claim doesn't already say it.
			if !strings.HasPrefix(purpose, "Not ported.") {
				statusSuffix = " Not yet ported."
			}
		}
		lines = append(lines, "- **"+displayName+"** — "+purpose+statusSuffix)
	}
