...
| `R-project-name-hotam-spec` | SETTLED | `framework-author` | A-python-stack | The project's name shall be Hotam-Spec (display) and hotam-spec (kebab-case for the filesystem, repository, and Go module path suffix), closing M1. |
...
| `R-presented-pending-decision-type` | SETTLED | `framework-reviewer` | — | The presented-awaiting-decision state shall live in a dedicated runtime folder structure (pending/ vs applied/), surfaced by the harness -- not as a new graph node type. |
