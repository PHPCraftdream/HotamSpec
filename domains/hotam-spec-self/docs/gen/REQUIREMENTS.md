...
| `R-axis-gatekeeper-policy` | SETTLED | `ai-agent` | — | Axis-duplicate gatekeeping shall be a mandatory part of the future axis-creation path (confront-style similarity check at creation time), not a separately-switched feature; until an axis-creation tool exists, the hand-curated axes tuple is the gate. |
