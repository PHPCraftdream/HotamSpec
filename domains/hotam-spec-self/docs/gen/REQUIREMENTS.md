...
| `R-context-budget-rule` | SETTLED | `framework-author` | A-finite-context-operators | An operator's owned domain shall not exceed its context budget (size(domain) <= budget.limit), with any excess flagged as a structural OVERLOADED contradiction by the harness. |
...
| `R-operator-crystal-embeds-thinking` | SETTLED | `framework-author` | A-python-stack | The operator's CLAUDE.md shall embed the full content of its scope-relevant thinking documentation inline, not as markdown links, so the operator holds the methodology itself rather than a table of contents. |
| `R-operator-crystal-embeds-tools` | SETTLED | `framework-author` | A-python-stack | The operator's CLAUDE.md shall embed the full content of its scope-relevant tool documentation inline, not as markdown links. |
...
|---|---|---|---|---|
| `OP-director` | `framework-author` | ACTIVE | 220 (NODE_COUNT) | — |
