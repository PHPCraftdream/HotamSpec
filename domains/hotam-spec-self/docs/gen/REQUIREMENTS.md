...
| `R-subagent-gets-its-claude-md` | SETTLED | `ai-agent` | A-finite-context-operators | A delegated sub-operator shall receive its OWN crystal (a CLAUDE.md generated from its sub-domain) and return CONCLUSIONS only, never raw context, to the root operator. |
