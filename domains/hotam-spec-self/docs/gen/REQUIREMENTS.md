...
| `R-context-hook-piggybacks-cah-stamp` | DRAFT | `framework-author` | A-finite-context-operators | The project-local hook (context_producer.py, wired via PostToolUse + Stop in .claude/settings.local.json) shall read a context-cache.json written by the global statusline script and write spec/.runtime/context.json. |
