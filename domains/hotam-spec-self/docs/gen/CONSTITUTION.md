...
**R-ai-presents-not-decides** — The AI agent shall NEVER close a Conflict silently -- it presents with justification and defers every resolution to the human resolver.

**R-resolver-distinct-from-owners** — Every Conflict's resolver shall be a Stakeholder who is NOT the owner of any of the conflict's members.

**R-operator-not-self-approve** — An Operator shall not resolver a Conflict in which its underlying Stakeholder owns one of the members.
...
| `R-conflict-is-connector-node` | ENFORCED | A contradiction shall be modeled as a first-class Conflict NODE carrying axis + context + shared_assumption + resolver, never as a `conflicts_with` edge between requirements. |
...
| **Hard boundary** | | |
| `R-ai-presents-not-decides` | STRUCTURAL | The AI agent shall NEVER close a Conflict silently -- it presents with justification and defers every resolution to the human resolver. |
| `R-resolver-distinct-from-owners` | ENFORCED | Every Conflict's resolver shall be a Stakeholder who is NOT the owner of any of the conflict's members. |
...
| `R-operator-not-self-approve` | ENFORCED | An Operator shall not resolver a Conflict in which its underlying Stakeholder owns one of the members. |
| **Self + delegation** | | |
...
| **Loop machinery** | | |
| `R-active-loop-playbooks` | PROSE | Each what_now priority band shall have a documented agent PLAYBOOK plus a tools/apply_proposal.py that mechanically applies a resolver-approved JSON proposal to spec/content/. |
...
| `R-uncrystallizable-is-missing-type` | STRUCTURAL | Knowledge an operator cannot crystallize as any existing node shall be RECORDED as a candidate missing ontology type for resolver review (not auto-acted). |
