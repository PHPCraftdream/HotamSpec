...
def build_graph() -> TensionGraph:
    ...
    requirements = (
        ...
        Requirement(
            ...
            settled_at="2026-06-30",
            created_at="2026-06-30",
            relations=(Relation("replaces", "R-binary-enforcement-gradient"),),
        ),
        Requirement(
        ...
        Requirement(
            ...
            status="SETTLED",
            why=(
                "Bookkeeping clarification only (etap E, task #107 inventory, lens-2-#3 + lens-4-#4) -- NOT a mechanism change. Confirms and extends the Wave 10 move 2 narrowing (2026-07-03) already recorded in this atom's why-text: the claim covers the TOOL MECHANISM of tools/spawn_agent.py, proven by test_spawn_log_written. The 2026-07-10 re-audit found the SAME empty-log symptom (spec/.runtime/spawn-log.jsonl carries one test record despite dozens of real host-level sub-agent spawns across this session) but traced it to a CATEGORY DISTINCTION the prior why-text implied but did not spell out: 'spawn' here means an explicit tools/spawn_agent.py invocation composing a domain sub-operator's CLAUDE.md crystal (R-agent-is-recursive-director, R-sub-agent-crystal-triad) -- there are currently ZERO such sub-operators in this domain (Agent Map: '(no sub-operators yet)'), so zero real invocations is the EXPECTED, honest state, not a broken rule. It is category-distinct from a host-level Agent-tool spawn (a Claude Code platform primitive, external to the framework's own code), which is covered by the separate, already-STRUCTURAL R-host-spawn-leaves-trace (hotam-dev) via the --log-only path. This proposal only makes that distinction explicit IN THE WHY (the claim itself stays a single atomic assertion, per R-requirement-claim-is-atomic -- an earlier draft folded the distinction into the claim text and tripped the atomicity ratchet test, test_no_new_compound_requirements_beyond_baseline) so a future reader does not re-derive the same 'spawn-log is empty, rule must be broken' conclusion a third time. No enforcement level change (stays ENFORCED on the same tool-mechanism tests); no new atom; Option B of the etap E inventory (see docs/reviews/2026-07-10-lens-2-simplification.md #3 and the task #107 PRESENT report)."
            ),
            assumptions=("A-finite-context-operators",),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            assumptions=("A-bootstrap-self-applies",),
            relations=(Relation("refines", "R-agent-never-lost"), Relation("refines", "R-ai-presents-not-decides"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            assumptions=("A-finite-context-operators",),
            relations=(Relation("refines", "R-context-bounded-delegation"), Relation("refines", "R-sub-agent-crystal-triad"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            claim=("The CONSTITUTION block in root CLAUDE.md shall render each SETTLED requirement as id + enforcement flag [E/S/P] grouped by category, with a pointer to docs/gen/REQUIREMENTS.md for full claims."),
            owner="framework-author",
            status="SETTLED",
            why=("The crystal is a seed plus indexes, not a catalog (R-crystal-reload-by-reference): full claims already live in the generated roster (docs/gen/REQUIREMENTS.md) and enforcement detail in docs/gen/UNENFORCED.md, so carrying them verbatim doubled the block (~38k chars of the 85k crystal). The index keeps every SETTLED anchor resident for the CONFRONT scan while cutting ~16k chars, and is orthogonal to Phase 3: relocating framework-plumbing atoms changes WHICH ids are listed, not the line format. R-operator-prompt-from-substrate stays true — all SETTLED requirements, grouped by category, generated deterministically. Phase 2.2b: claim text removed from index lines — the claim was truncated to 96 chars with ellipsis, contradicting R-crystal-carries-short-form; short_form(claim, summary) is the correct mechanism and lives in REQUIREMENTS.md."),
            assumptions=("A-finite-context-operators", "A-compaction-loses-working"),
            relations=(),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            relations=(Relation("refines", "R-anchor-everything"), Relation("refines", "R-drift-structurally-impossible"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            claim=("Every SETTLED/ENFORCED requirement's enforced_by entries shall resolve to a concrete pytest node-id via the shared enforcer-resolution algorithm, and every bare test_* enforcer shall have real assertions (not a pass-only stub)."),
            owner="framework-author",
            status="SETTLED",
            why=("Wave 1 (seed coherence audit): check_enforced_names_invariant only checks that enforced_by is NON-EMPTY -- a real typo in an entry (a renamed test file, a stale check_* name) passes that check silently and is only discovered when someone runs tools/gate.py by hand and it fails closed to the full suite. That is an honest but SILENT tax on the T1 tiered gate (R-verify-closure-per-action): the graph claims a targeted enforcer exists when it does not, and the steward never sees this as debt. check_enforced_by_resolvable makes the debt visible: it AST-parses tests/*.py to build the check_* -> test-file map (the same map gen_spec.py's CONCEPT-MAP block and tools/gate.py's T1 selector both build) and resolves every enforced_by entry against it, firing a Violation naming the exact unresolvable string. check_enforced_by_test_has_teeth is the companion lint: a test that exists but whose body is only pass/docstring (no assert, no call, no raise) is toothless -- it makes ENFORCED a lie. The resolution algorithm itself was extracted out of tools/gate.py into spec/src/hotam_spec/enforcer_resolution.py so this invariant (which lives in src/ and must not import from tools/) and gate.py (a tool) share ONE implementation rather than two hand-synced copies (R-prefer-tool-over-hand). AST-based, in-process, no subprocess spawn -- deterministic within a run."),
            relations=(Relation("refines", "R-verify-closure-per-action"), Relation("refines", "R-enforcement-first-class"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            owner="framework-author",
            status="SETTLED",
            why=("M5, REPLACES split into R-trust-anchor-mechanism + R-trust-anchor-delegation-explicit-only per atomicity discipline (R-requirement-claim-is-atomic). Steward verdict 2026-07-02 (verbatim): «человек обязан сам подписать, если только явно не делегирует это в каждом случае или на кампанию вперед» (English: 'the human is obliged to sign personally, unless he explicitly delegates it, either per-case or for a campaign in advance'). This atom lands the EXCEPTION half: delegation is not the default and is not implicit from e.g. an agent being given repo write access -- it requires an explicit steward grant, scoped either to one case or to a named campaign declared in advance. RETIRED MECHANISM (steward triage decision B1, 2026-07-09, verbatim: «снести старую jsonl-систему делегаций полностью»): the original mechanism -- domains/hotam-spec-self/delegations.jsonl, a durable committed append-only JSONL ledger of {id, steward, verbatim, date, scope} records written by spec/tools/record_delegation.py, cross-checked against Conflict.lifecycle markers by spec/tests/test_delegation_marker_honesty.py -- has been torn down completely: the tool, its CLI wrapper, its tests, the marker-honesty cross-check, and the ledger file itself are all removed. The seed record DEL-1 (steward domain-user, date 2026-07-02, scope 'campaign: session pipeline decisions (C-be22cdd1 HELD + DECIDED)') that anchored the core-vs-aspect Conflict's DECIDED transition is preserved verbatim, not lost, in docs/development/DELEGATIONS-HISTORY.md and in the Conflict's own Signoff.instrument=\"DEL-1\" field (R-signoff-preserved-in-substrate) and lifecycle marker text -- both left as inert historical record, no longer mechanically cross-checked against a live ledger. CURRENT MECHANISM: this claim is now upheld by the SAME file-based mechanism R-delegation-is-a-file already established for every task hand-off -- a delegation (whether an operational task hand-off or a personal-signature campaign grant) is a versioned delegations/DG-<n>.md file, created and closed only via spec/tools/delegate.py, with git history as the audit trail; no dedicated ledger format or writer tool is needed for the signature-delegation case specifically. ENFORCED by the same tests that cover the file mechanism: spec/tests/test_tool_delegate.py (create allocates id + writes file; close sets done + records result) -- shared enforcers across R-delegation-is-a-file and this atom are acceptable per check_bijection_r_to_enforcer's documented SHARED-enforcer allowance."),
            assumptions=("A-stakeholders-care", "A-bootstrap-self-applies"),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            relations=(Relation("refines", "R-conflict-is-connector-node"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            relations=(Relation("refines", "R-stale-substrate"),),
            enforcement="ENFORCED",
        ...
        Requirement(
            ...
            claim=("While the context cipher is UNMEASURED, the generated LIVE-STATE shall honestly explain that measuring working-context requires host cooperation the framework will not touch, naming no command-to-call."),
            owner="ai-agent",
            status="SETTLED",
            why=("REPLACES R-unmeasured-cipher-names-user-action (steward verdict 2026-07-05). Measuring working-context would require the sovereign host's statusline / global ~/.claude to deliver ctx_pct — cooperation the framework will not reach for (R-work-within-launch-dir). The prior atom demanded the UNMEASURED line name the exact command to run; the only such command (setup_context_hook.py --patch-global --apply) patched the host and is now removed. Naming a nonexistent in-bounds command would be dishonest, so the cipher instead states the host boundary plainly: tools/context.py render_line() embeds the honest explanation, and gen_spec's LIVE-STATE + emit_cipher surface it verbatim. The cipher measures only if the local stdin payload honestly carries ctx_pct (R-work-within-launch-dir)."),
            assumptions=("A-finite-context-operators",),
            enforcement=ENFORCED,
            enforced_by=("test_tool_context.py::test_absent_stamp_reads_unmeasured", "test_tool_context.py::test_stamp_without_pct_renders_unmeasured_line",),
            settled_at="2026-07-05",
        ...
        Requirement(
            ...
            owner="framework-author",
            status="SETTLED",
            why=("Names the legitimate resolution surface (amend members, add a mediating entity, or both) so 'solved in the chat / in the code, conflict left dangling' is a visible violation of the connector-node discipline (R-conflict-is-connector-node). Steward verdict 2026-07-05: 'противоречия либо решают внутри конфликтующих объектов, либо дополнительным созданием сущности' -- the paths are not exclusive. Labeled INHERENTLY_PROSE 2026-07-10 (Этап T): the claim's essence -- that a resolution happened INSIDE the graph and not somewhere in the real world -- requires comparing the graph against the world, which no check_* can do; any 'enforcer' would only measure form (the R-boot-cite-measured form-metric theatre, REJECTED). The honestly-checkable remainder is already covered by check_decided_has_rationale_or_derived + the signoff checks (a DECIDED conflict must carry rationale-or-derived and a human signoff). This is permanent discipline, not closeable debt."),
            enforcement="PROSE",
            settled_at="2026-07-05",
            created_at="2026-07-05",
            enforceability="INHERENTLY_PROSE",
            history=(HistoryEntry(at="2026-07-10", summary="why: Steward verdict 2026-07-05, verbatim: '…→Names the legitimate resolution surface…; enforceability: ENFORCEABLE→INHERENTLY_PROSE"),),
        ),
        Requirement(
        ...
        Requirement(
            ...
            claim=("At the CONFRONT step the operator shall check every change not only against SETTLED rules but also against the graph's goals, IMPLEMENTS aspirations and values, surfacing any contradiction before writing."),
            owner="framework-author",
            status="SETTLED",
        ...
        Requirement(
            ...
            owner="framework-author",
            status="SETTLED",
            why=("Today spec/ and its own domains (domains/hotam-spec-self/, domains/hotam-dev/) share one repo, and the core assumes this coincidence: 13+ call sites compute the 'consumer root' as spec_root().parent (Path(__file__).resolve().parents[N]). This blocks using HotamSpec as tooling for a separate project (e.g. a domain 'prat' living in D:/ai_dev/prat/) -- the framework forces that domain to live inside D:/dev/HotamSpec/domains/prat/, inside the framework's own repo, instead of the consumer's project. This requirement makes the coincidence OPTIONAL: self-hosting remains a legitimate, fully-tested working case (R6 fallback, R-domain-self-hosting-flag unaffected), but every path that reads/writes CONSUMER content (domains/, tickets/, delegations/, CLAUDE.md, .claude/settings.json, runtime state) is derived from project_root(), resolved once per call through the documented priority chain, never guessed from __file__. This is philosophically continuous with R-work-within-launch-dir (the framework only ever acts within the directory it was invoked for, never reaches outside it) -- portability generalizes that same discipline to a project root that need not equal the framework's own install location. Full specification (priority chain rationale R1-R6, complete file inventory, 4-wave migration path W1-W4 with acceptance criteria AC1-AC10, package-data/entry-point layout): D:/ai_dev/prat/REQUIREMENTS-hotamspec-portable.md. Steward verdict 2026-07-09: accept, land as SETTLED, begin wave W1. ENFORCEMENT (Etap V, #124, 2026-07-10) -- STRUCTURAL->ENFORCED via a narrow syntactic check, not theatre: tests/test_project_root_not_hardcoded.py AST-scans every committed framework .py (spec/tools/ + spec/src/hotam_spec/, top level) and fails RED if any file reaches the consumer's data by GUESSING the root from __file__ -- i.e. a Path(__file__).resolve().parents[N] climb (N>=2) or an equivalent .parent.parent... chain (depth>=2) joined in the SAME expression to a consumer-data segment (domains, tickets, delegations, CLAUDE.md, .claude, docs/gen). It is the JOIN of a __file__-climb to consumer data that constitutes the forbidden guess; a shallow framework-internal parents[1] (a tool locating spec/ to fix sys.path) and repo_paths' own framework-root climb (which serves framework-internal paths, never consumer data) are correctly NOT flagged. The live tree has ZERO offenders because the W1 migration already routed every consumer-root read through project_root_or_raise() (_ticket_store.py / _delegation_store.py / claude_md_diff_watch.py / repo_paths.py all delegate to the R1-R6 resolver); the enforcer nails that steady state so no future commit can silently reintroduce a hardcoded consumer-root climb. Honest scope, documented in the enforcer's docstring: top-level *.py only (matching the sibling R-work-within-launch-dir / R-core-periphery scanners; cli/_path_setup.py computes a framework-internal bundled _tools path, not consumer data, and is out of the swept set anyway); no cross-statement data-flow (the climb and the consumer-data join must appear in one expression, the exact literal shape the claim names). project_paths.py (the single sanctioned R1-R6 resolver) is allowlisted. A non-vacuity negative control (test_scanner_catches_a_hardcoded_root_negative_control) proves the predicate flags four synthetic hardcoded-root guesses (parents[2]/'domains', parents[3]/'CLAUDE.md', .parent.parent/'tickets', parents[2]/'docs'/'gen') and passes four in-bounds shapes (shallow spec/ climb, deep climb joined to non-consumer _tools, consumer path from project_root_or_raise(), bare deep climb with no consumer join)."),
            enforcement="ENFORCED",
            created_at="2026-07-09",
            settled_at="2026-07-10",
            enforced_by=("test_project_root_not_hardcoded.py::test_no_committed_code_guesses_consumer_root_from_file", "test_project_root_not_hardcoded.py::test_scanner_catches_a_hardcoded_root_negative_control"),
            history=(HistoryEntry(at="2026-07-10", summary="why: Today spec/ and its own domains (domain…→Today spec/ and its own domains (domain…; enforcement: STRUCTURAL→ENFORCED; enforced_by: []→[test_project_root_not_hardcoded.py::te…; settled_at: 2026-07-09→2026-07-10"),),
        ),
        Requirement(
        ...
        Requirement(
            ...
            claim=("The three-level enforcement gradient (PROSE/STRUCTURAL/ENFORCED) shall be collapsed into a binary ENFORCED/NOT_ENFORCED distinction plus a separate inherently_prose flag, because the middle STRUCTURAL tier is redundant."),
            owner="framework-author",
            status="REJECTED",
