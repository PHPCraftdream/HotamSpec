...
| `R-domain-owns-docs-gen` | SETTLED | `framework-author` | A-python-stack | Each `domains/<name>/docs/gen/` holds the markdown generated from that domain's graph; no cross-domain doc files. |
| `R-domain-owns-tools-and-agents` | SETTLED | `framework-author` | A-python-stack | Each `domains/<name>/` owns its `tools/` (private tools) and its `agents/` (sub-operators); both directories MUST exist (may be empty). |
...
| `R-framework-claude-md-is-domain-free` | SETTLED | `framework-author` | A-python-stack | The root `CLAUDE.md` contains framework-only content and references domains only through the DOMAIN-MAP block; no domain-specific atoms appear in it. |
