...
| `R-operator-type-vs-facet` | SETTLED | `framework-author` | A-bootstrap-self-applies | Operator shall be its own first-class frozen-dataclass type in tensio.operator (not a Stakeholder facet), with typed anchor 'OP-', a ContextBudget, and an optional parent reference. |
