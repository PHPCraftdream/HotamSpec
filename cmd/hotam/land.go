...
// the standalone commands (applyProposalFile / genSpec / allViolations) so
// its behavior is provably identical to running them one at a time:
//
//  1. apply the proposal (strict decode; internal/proposal.Apply rejects the
//     write outright if the mutated graph would introduce new invariant
//     violations — see internal/proposal/apply.go).
...
func cmdLand(args []string) error {
	...
	if fs.NArg() < 1 {
		return fmt.Errorf("usage: hotam land <proposal.json> --domain <path> --today YYYY-MM-DD")
	}
	if *domain == "" {
	...
	// (a) apply-proposal: strict parse + invariant-checked write.
