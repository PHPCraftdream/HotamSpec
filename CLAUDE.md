...
Operator of `hotam-spec-self` (178 SETTLED). Guardian: **spec** (`domains/hotam-spec-self/graph.py`) ↔ **tests** (`check_*`/`test_*`) ↔ **business** (steward decisions). Drift between layers = top signal.
...
- **top action:** [P0] REFLECTION on `enforcement-gradient` — 16 SETTLED requirements are closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL) — claimed but not guaranteed, soft context-debt. See docs/gen/UNENFORCED.md.
- **debt:** 149/178 SETTLED ENFORCED · 12 DRAFT · 2 OPEN · 16 closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL)
- **graph:** 232 nodes (req+conflict+assumption); OP-director budget 150000 chars (CRYSTAL_CHARS measure) — resident crystal 55005 chars (headroom 94995)
