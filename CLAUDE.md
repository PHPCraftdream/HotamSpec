...
- **debt:** 112/147 SETTLED ENFORCED · 12 DRAFT · 7 OPEN · 35 SETTLED-unenforced
- **graph:** 200 nodes (req+conflict+assumption); OP-director budget 220 (headroom 20)
...
1. Emit a `Proposed*` JSON object — `ProposedRequirement` / `ProposedConflictTransition` / `ProposedRejection` (`tensio.proposal`, `R-active-loop-playbooks`).
...
- **R-operator-is-frozen-dataclass** — *An Operator shall be a frozen dataclass in tensio.operator with typed anchor 'OP-'.* [ENFORCED·check_typed_anchors_operator·test_operator.py]
...
- **R-operator-type-vs-facet** — *Operator shall be its own first-class frozen-dataclass type in tensio.operator (not a Stakeholder facet), with typed anchor 'OP-', a ContextBudget, and an optional parent reference.* [ENFORCED·check_typed_anchors_operator·test_operator.py]
...
- **R-content-free-no-business-data** — *The framework spec/src/tensio/ shall ship no business data (no example requirements, no example axes, no business stakeholders).* [ENFORCED·test_content_free.py::test_no_domain_instances_in_tensio_src]
...
- **R-method-matches-docstring** — *Each check_* function in tensio.invariants.ALL_INVARIANTS shall have a docstring whose RULE line shares non-trivial lexical overlap with its body's Violation messages.* [ENFORCED·check_method_matches_docstring]
