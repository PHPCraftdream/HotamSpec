...
<!-- DOMAIN-CRYSTAL:BEGIN -->
<!-- (generated by tools/gen_spec.py — do not hand-edit) -->

### Domain crystal (substrate-written operator prompt)

_Below is the full content of the active domain's `CLAUDE.md` (`domains/hotam-spec-self/CLAUDE.md`) — the canonical entry-point of the domain AND the base from which every sub-agent derives its scoped CLAUDE.md. When Claude Code auto-loads root CLAUDE.md, this section IS the operator's prompt; substrate writes operator (R-operator-prompt-from-substrate)._
...
- **§Conflict**
  ...
  - tested: `spec/tests/test_conscience.py`, `spec/tests/test_docs_gen.py`, `spec/tests/test_invariants.py`, `spec/tests/test_latent_connector_noise_fix.py`
- **§Conscience**
  - defined: `_(not yet mapped)_`
  - enforced: _(none)_
