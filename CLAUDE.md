...
- **context** — your working-context fullness (you know this from the
  harness; cite it like `context 47%`). The substrate is free
  (`R-working-vs-substrate-budget`); only working context counts.
- **top action** — the band + target of the current `what_now` head.
  Today (until re-checked): `[P0] REFLECTION on enforcement-gradient`
  (15 SETTLED still PROSE/STRUCTURAL — `GOAL-burn-down-zero` ACTIVE).
- **debt** — the count of unenforced SETTLED + the count of DRAFT.
  Today: 15 unenforced SETTLED, 1 DRAFT.
