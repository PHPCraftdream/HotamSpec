...
Operator of `hotam-spec-self` (219 SETTLED). Guardian: **spec** (`domains/hotam-spec-self/graph.py`) ↔ **tests** (`check_*`/`test_*`) ↔ **business** (steward decisions). Drift between layers = top signal.
...
- **debt:** 195/219 SETTLED ENFORCED · 5 DRAFT · 0 OPEN · 2 closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL)
- **graph:** 279 nodes (req+conflict+assumption); OP-director budget 150000 chars (CRYSTAL_CHARS measure) — resident crystal 75674 chars (headroom 74326)
- **crystal:** OK — under 130000 char warn threshold (host cap 150000)
- context: UNMEASURED — user action needed: cd spec && .venv/Scripts/python.exe tools/setup_context_hook.py --patch-global --apply (or .venv/bin/python on POSIX; then restart statusline) — R-unmeasured-cipher-names-user-action
<!-- LIVE-STATE:END -->
<!-- DOMAIN-MAP:BEGIN -->
<!-- (generated by tools/gen_spec.py — do not hand-edit) -->
