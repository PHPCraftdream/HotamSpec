...
Operator of `hotam-spec-self` (165 SETTLED). Guardian: **spec** (`domains/hotam-spec-self/graph.py`) ↔ **tests** (`check_*`/`test_*`) ↔ **business** (steward decisions). Drift between layers = top signal.
...
- **top action:** [P0] REFLECTION on `enforcement-gradient` — 11 SETTLED requirements are closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL) — claimed but not guaranteed, soft context-debt. See docs/gen/UNENFORCED.md.
- **debt:** 141/165 SETTLED ENFORCED · 12 DRAFT · 8 OPEN · 11 closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL)
- **graph:** 225 nodes (req+conflict+assumption); OP-director budget 150000 chars (CRYSTAL_CHARS measure) — resident crystal 54512 chars (headroom 95488)
