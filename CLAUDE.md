...
Operator of `hotam-spec-self` (232 SETTLED). Guardian: **spec** (`domains/hotam-spec-self/graph.py`) ↔ **tests** (`check_*`/`test_*`) ↔ **business** (steward decisions). Drift between layers = top signal.
...
- **debt:** 203/232 SETTLED ENFORCED · 2 DRAFT · 0 OPEN · 4 closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL)
- **graph:** 298 nodes (req+conflict+assumption); OP-director budget 150000 chars (CRYSTAL_CHARS measure) — resident crystal 27664 chars (headroom 122336)
...
- **open actions** — 16 (top: [P7] R-axes-as-module-constant: REJECTED requirement 'R-axes-as-module-constant' claims a REPLACES successor in prose but has NO structural `replaces` edge — migrate it ...)
<!-- DOMAIN-MAP:END -->
<!-- CONSTITUTION:BEGIN -->
<!-- (generated by tools/gen_spec.py — do not hand-edit) -->
...
| **§Attention** | `spec/src/hotam_spec/attention.py` | _(none)_ | 3 tests |
| **§Axis** | `spec/src/hotam_spec/axis.py` | 1 checks | 3 tests |
| **§Closure** | `spec/src/hotam_spec/enforcer_resolution.py` | 2 checks | 3 tests |
| **§Conflict** | `spec/src/hotam_spec/conflict.py` | 17 checks | 7 tests |
...
| **§Graph** | `spec/src/hotam_spec/graph.py` | _(none)_ | 4 tests |
| **§Invariants** | `spec/src/hotam_spec/invariants.py` | 47 checks | 27 tests |
...
| **§Requirement** | `spec/src/hotam_spec/requirement.py` | 13 checks | 9 tests |
...
- **R-active-loop-playbooks** (REJECTED) — REPLACES by R-active-loop-protocol, R-active-loop-apply-tool, R-active-loop-playbook-doc
- **R-agent-has-own-crystal** (REJECTED) — REPLACES by R-claude-md-consolidates-when-single-agent, R-sub-agent-crystal-triad — (was: The agent's prompt is independent of the director's. BUILD-TRIGGER: same as R-agent-is-a-directory. Promoted DRAFT→SETTLED on firs…
- **R-agent-imports-framework** (REJECTED) — REPLACES by R-agent-code-imports-framework, R-framework-owned-by-no-agent — (was: Keeps the framework content-free (R-content-free-framework) while letting agents specialize. BUILD-TRIGGER: same as R-agent-is-a-directory…
...
- **R-boot-from-substrate** (REJECTED) — REPLACES by R-boot-reload-three-facts, R-boot-cite-in-first-sentence — (was: REJECTED — REPLACES split into R-boot-reload-three-facts + R-boot-cite-in-first-sentence (wave 2, decided by framework-author 2026-06-30) — (wa…
- **R-claude-md-budget-phi-cap** (REJECTED) — REPLACES by R-working-vs-substrate-budget, R-budget-measure — (was: The context-bounded-delegation law (R-context-bounded-delegation) applied to the operator's OWN body, not just the graph. BUILD-TRIGGER: CLAUDE.md cross…
- **R-content-free-framework** (REJECTED) — REPLACES by R-content-free-no-business-data, R-content-free-no-examples, R-content-free-no-seed-graph — (was: Hotam-Spec is a blank kit. Business content lives under spec/content/; the worked example is a test fixture. R…
...
- **R-critical-core-scope** (REJECTED) — REPLACES by R-critical-core-methodology, R-critical-core-per-domain — (was: REJECTED — REPLACES split into R-critical-core-methodology + R-critical-core-per-domain (wave 2, decided by framework-author 2026-06-30) — (was:…
- **R-dependency-graph-parallelism** (REJECTED) — REPLACES by R-dependency-tracked, R-dependency-drives-parallel, R-dependency-drives-sequential — (was: REJECTED — REPLACES split into R-dependency-tracked + R-dependency-drives-parallel + R-dependency-drives-sequential (…
...
_(showing 12 of 39, alphabetical by id — full history: docs/gen/HISTORY.md)_
