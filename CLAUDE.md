...
Operator of `hotam-spec-self` (207 SETTLED). Guardian: **spec** (`domains/hotam-spec-self/graph.py`) ↔ **tests** (`check_*`/`test_*`) ↔ **business** (steward decisions). Drift between layers = top signal.
...
- **top action:** [P4] OPEN_ITEM on `A-bootstrap-self-applies` — review assumption 'A-bootstrap-self-applies' ("The framework can model its own design coherently — Hotam-Spec's own methodology fits its own ontology with no special-casing."): still UNCERTAIN with 58 dependent requirements — resolve the doubt (transition to DEAD or re-affirm HOLDS) or it drifts
- **debt:** 187/207 SETTLED ENFORCED · 5 DRAFT · 0 OPEN · 0 closeable debt (ENFORCEABLE, still PROSE/STRUCTURAL)
- **graph:** 264 nodes (req+conflict+assumption); OP-director budget 150000 chars (CRYSTAL_CHARS measure) — resident crystal 67726 chars (headroom 82274)
...
- **open actions** — 4 (top: [P3] C-80687260: conflict 'C-80687260' on axis 'sequential-vs-isolated-parallel' is DETECTED with no steward movement; steward 'dev-steward' must ACKNOWLE...)
...
- **atoms-count** — 207 SETTLED
- **open actions** — 3 (top: [P4] A-bootstrap-self-applies: review assumption 'A-bootstrap-self-applies' ("The framework can model its own design coherently — Hotam-Spec's own methodology fits its ...)
<!-- DOMAIN-MAP:END -->
<!-- CONSTITUTION:BEGIN -->
<!-- (generated by tools/gen_spec.py — do not hand-edit) -->
...
- R-unmeasured-cipher-names-user-action — While the context cipher is UNMEASURED, the generated LIVE-STATE shall name the exact user-run c… [E]
...
- **§Requirement**
  ...
  - tested: `spec/tests/test_conscience.py`, `spec/tests/test_decisions_bijection.py`, `spec/tests/test_docs_gen.py`, `spec/tests/test_enforceability.py`, `spec/tests/test_invariants.py`, `spec/tests/test_no_observation_type_scope.py`
- **§Scope**
...
_(showing 12 of 26, alphabetical by id — full history: docs/gen/HISTORY.md)_
