...
- **debt:** 114/150 SETTLED ENFORCED · 12 DRAFT · 7 OPEN · 27 SETTLED-unenforced
- **graph:** 207 nodes (req+conflict+assumption); OP-director budget 220 (headroom 13)
...
- **§Conflict**
  ...
  - tested: `spec/tests/test_conscience.py`, `spec/tests/test_docs_gen.py`, `spec/tests/test_embedded_thinking_tools.py`, `spec/tests/test_invariants.py`, `spec/tests/test_latent_connector_noise_fix.py`
- **§Conscience**
  - defined: `_(not yet mapped)_`
  - enforced: _(none)_
...
_Full content of every `spec/docs/thinking/*.md` file, embedded so the sole operator boots from substrate without a second file read._
...
_Full content of every `spec/docs/tools/*.md` file, embedded so the sole operator boots from substrate without a second file read. Unfiltered — all tools are in scope for the sole operator._
