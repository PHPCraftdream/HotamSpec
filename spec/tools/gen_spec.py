...
def _render_constitution_block(g: TensionGraph) -> str:
    ...
    # Append entity-derived requirements (R-entity-derived-requirement projection).
    if entity_section:
...
def _scope_relevant_thinking_topics(
    scope_prefixes: tuple[str, ...],
    g: TensionGraph,
    thinking_dir: Path | None = None,
) -> list[str]:
    """Return sorted thinking-doc topic slugs relevant to an operator's SCOPE.

    - scope_prefixes == () (director / unscoped): ALL topic slugs that have a
      corresponding spec/docs/thinking/<slug>.md file.
    - Otherwise: render the SCOPE-filtered CONSTITUTION block (reusing
      _render_scoped_constitution_block, the same filter the operator's
      CONSTITUTION digest already uses) and treat a topic as relevant if its
      capitalized name (e.g. "Conflict" for slug "conflict") appears
      case-insensitively anywhere in that rendered text.

    Safety net: if literally zero topics match (e.g. a scope whose prefixes
    happen not to mention any §Topic name in its digest text), fall back to
    returning ALL topics — an operator crystal with no methodology content at
    all is a worse failure mode than one with too much.
    """
    _thinking = thinking_dir or SPEC_DOCS_THINKING_DIR
    all_slugs = (
        sorted(p.stem for p in _thinking.glob("*.md")) if _thinking.exists() else []
    )
    ...
    tool_reqs = _scan_tool_requirements()
    scoped_text = _render_scoped_constitution_block(g, scope_prefixes, tool_reqs)
    scoped_text_lower = scoped_text.lower()

    relevant = [
        slug
        for slug in all_slugs
        if slug.replace("-", " ").title().replace(" ", "").lower() in scoped_text_lower
    ]
    if not relevant:
        # Safety net: no topic name matched the scoped digest text — emit all
        # topics rather than leave the operator with zero methodology content.
        return all_slugs
    return sorted(relevant)


def _render_embedded_thinking_block(
    scope_prefixes: tuple[str, ...],
    g: TensionGraph,
    thinking_dir: Path | None = None,
) -> str:
    """Render the EMBEDDED-THINKING block: FULL BODY of scope-relevant thinking docs.

    Unlike SHARED-DOCS (a link index Claude Code does not follow when loading
    CLAUDE.md), this embeds the actual prose so the operator crystal holds the
    methodology itself, not a table of contents. Deterministic: topics sorted
    alphabetically by slug, content read verbatim via `_read`.
    """
    _thinking = thinking_dir or SPEC_DOCS_THINKING_DIR
    slugs = _scope_relevant_thinking_topics(scope_prefixes, g, _thinking)
    ...
    lines: list[str] = [
        ...
        "",
        "_The operator's crystal embeds the full body of its scope-relevant "
        "thinking docs below — not links — so the operator holds the "
        "methodology itself, not a table of contents._",
        "",
    ]

    if not slugs:
        lines.append("_(no thinking docs available)_")
        return "\n".join(lines).rstrip()

    for slug in slugs:
        path = _thinking / f"{slug}.md"
        if not path.exists():
            continue
        topic_label = "§" + slug.replace("-", " ").title().replace(" ", "")
        content = _read(path)

        # Strip the file's own leading "<!-- Auto-generated ... -->" comment
        # line and its "# Canon — §Topic" H1 (both duplicate the #### §Topic
        # heading emitted below); keep the rest of the body verbatim.
        body_lines = content.split("\n")
        idx = 0
        # Skip a single leading HTML-comment line.
        if idx < len(body_lines) and body_lines[idx].strip().startswith(
            "<!-- Auto-generated"
        ):
            idx += 1
        # Skip one blank line after the comment.
        while idx < len(body_lines) and body_lines[idx].strip() == "":
            idx += 1
        # Skip the "# Canon — §Topic" H1 line, if present.
        if idx < len(body_lines) and body_lines[idx].startswith("# "):
            idx += 1
...
def _substitute_embedded_thinking_block(text: str, new_block: str) -> str:
    """Substitute (or insert) the EMBEDDED-THINKING sentinel block into `text`.

    Mirrors the SHARED-DOCS substitution pattern: if sentinels are present,
    replace their inner content; otherwise insert immediately after
    SHARED-DOCS:END (the EMBEDDED-THINKING block always follows SHARED-DOCS).
    """
    if _EMBEDDED_THINKING_BEGIN in text and _EMBEDDED_THINKING_END in text:
        begin_pos = text.find(_EMBEDDED_THINKING_BEGIN)
        end_pos = text.find(_EMBEDDED_THINKING_END)
        before = text[: begin_pos + len(_EMBEDDED_THINKING_BEGIN)]
        after = text[end_pos:]
        return before + "\n" + new_block + "\n" + after

    shared_end_pos = text.find(_SHARED_DOCS_END)
    if shared_end_pos != -1:
        insert_at = shared_end_pos + len(_SHARED_DOCS_END)
        return (
            text[:insert_at]
...
def _update_agent_shared_docs_block(
    ...
    Also rewrites the EMBEDDED-THINKING block that immediately follows it
    (the full-content counterpart of the SHARED-DOCS link index).
    """
    claude_md_path = agent_dir / "CLAUDE.md"
    if not claude_md_path.exists():
        return
    ...
    # --- EMBEDDED-THINKING (full-content counterpart of SHARED-DOCS) ---
    embedded_block = _render_embedded_thinking_block(scope, g, thinking_dir)
    new_text = _substitute_embedded_thinking_block(new_text, embedded_block)
...
def _regenerate_agent_shared_docs(
    ...
    for agent_dir in sorted(root.iterdir()):
        ...
        _update_agent_shared_docs_block(
            agent_dir, scope, g, thinking_dir, tools_dir, spec_tools_dir
        )
...
# ---------------------------------------------------------------------------
# DOMAIN-CRYSTAL block — embed active domain's CLAUDE.md in root CLAUDE.md
# ---------------------------------------------------------------------------


def _render_domain_crystal_block() -> str:
    """Render the DOMAIN-CRYSTAL block content (without sentinels).

    Finds the active domain's CLAUDE.md (the domain's canonical entry-point,
    NOT the director agent's CLAUDE.md) via _active_domain() and embeds its
    full content. Strips inner DOMAIN-CRYSTAL sentinels to avoid recursive
    embedding. If the domain CLAUDE.md is absent, emits a calm placeholder.
    """
    active = _active_domain()
    lines: list[str] = [
        "<!-- (generated by tools/gen_spec.py — do not hand-edit) -->",
        "",
        "### Domain crystal (substrate-written operator prompt)",
        "",
    ]
    if active is None:
        lines.append(
            "_No active domain found — domain crystal unavailable. "
            "Run `uv run python tools/gen_spec.py` after populating `domains/`._"
        )
        return "\n".join(lines).rstrip()

    domain_claude_md = active / "CLAUDE.md"
    domain_name = active.name

    if not domain_claude_md.exists():
        lines.append(
            f"_Domain CLAUDE.md not found at `domains/{domain_name}/CLAUDE.md`. "
            "Create the domain scaffold and re-run gen_spec.py._"
        )
        return "\n".join(lines).rstrip()

    lines.append(
        f"_Below is the full content of the active domain's `CLAUDE.md` "
        f"(`domains/{domain_name}/CLAUDE.md`) — the canonical entry-point of the domain "
        f"AND the base from which every sub-agent derives its scoped CLAUDE.md. When Claude Code "
        f"auto-loads root CLAUDE.md, this section IS the operator's prompt; substrate writes "
        f"operator (R-operator-prompt-from-substrate)._"
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    domain_content = _read(domain_claude_md)
    # Strip inner DOMAIN-CRYSTAL sentinels to avoid recursive embedding.
    domain_content = domain_content.replace(_DOMAIN_CRYSTAL_BEGIN, "").replace(
        _DOMAIN_CRYSTAL_END, ""
    )
    lines.append(domain_content.rstrip())
    return "\n".join(lines).rstrip()


def _update_claude_md_domain_crystal() -> None:
    """Rewrite (or insert) the DOMAIN-CRYSTAL sentinel block in root CLAUDE.md.

    Position: AFTER THINKING-INDEX:END. Idempotent.
    """
    if not CLAUDE_MD.exists():
        return
    text = _read(CLAUDE_MD)
    # Migrate any legacy DIRECTOR-CRYSTAL sentinels to DOMAIN-CRYSTAL.
    if "<!-- DIRECTOR-CRYSTAL:BEGIN -->" in text:
        text = text.replace("<!-- DIRECTOR-CRYSTAL:BEGIN -->", _DOMAIN_CRYSTAL_BEGIN)
        text = text.replace("<!-- DIRECTOR-CRYSTAL:END -->", _DOMAIN_CRYSTAL_END)
    new_block = _render_domain_crystal_block()

    if _DOMAIN_CRYSTAL_BEGIN in text and _DOMAIN_CRYSTAL_END in text:
        begin_pos = text.find(_DOMAIN_CRYSTAL_BEGIN)
        end_pos = text.find(_DOMAIN_CRYSTAL_END)
        before = text[: begin_pos + len(_DOMAIN_CRYSTAL_BEGIN)]
        after = text[end_pos:]
        new_text = before + "\n" + new_block + "\n" + after
        _write(CLAUDE_MD, new_text)
    else:
        # Insert after THINKING-INDEX:END if present, else append.
        pos = text.find(_THINKING_INDEX_END)
        if pos != -1:
            insert_at = pos + len(_THINKING_INDEX_END)
            new_text = (
                text[:insert_at]
                + f"\n\n{_DOMAIN_CRYSTAL_BEGIN}\n{new_block}\n{_DOMAIN_CRYSTAL_END}\n"
                + text[insert_at:]
            )
        else:
            new_text = (
...
def _render_recently_rejected_block(g: TensionGraph) -> str:
    ...
    Lists every REJECTED requirement whose why contains 'REJECTED — REPLACES',
    ...
    for r in rejected_with_replaces:
        # Extract short summary: everything up to first em-dash or end
        why_short = r.why  # noqa: F841
        # Try to extract "REPLACES <X>" part
        replaces_part = ""
        if "REPLACES" in r.why:
            replaces_start = r.why.index("REPLACES")
            # Extract until the next em-dash, parenthesis or end
        ...
        # Short why: first 120 chars
        why_truncated = r.why[:120].replace("\n", " ").strip()
        if len(r.why) > 120:
            why_truncated += "…"
        entry = f"- **{r.id}** (REJECTED)"
        if replaces_part:
            entry += f" — {replaces_part}"
        entry += f" — _{why_truncated}_"
        lines.append(entry)
...
def _update_claude_md_recently_rejected(g: TensionGraph) -> None:
    ...
    Position: AFTER DIRECTOR-CRYSTAL:END. Idempotent.
    """
    if not CLAUDE_MD.exists():
        return
    text = _read(CLAUDE_MD)
    ...
    if _RECENTLY_REJECTED_BEGIN in text and _RECENTLY_REJECTED_END in text:
        ...
        # Insert after DIRECTOR-CRYSTAL:END if present, else after THINKING-INDEX:END, else append.
        for sentinel in (_DOMAIN_CRYSTAL_END, _THINKING_INDEX_END):
            pos = text.find(sentinel)
            if pos != -1:
                insert_at = pos + len(sentinel)
                new_text = (
...
def _update_domain_claude_md_all_blocks(domain_dir: Path, g: TensionGraph) -> None:
    ...
    # --- EMBEDDED-THINKING (full-content counterpart of SHARED-DOCS) ---
    # Director-level (domain root) is unscoped (scope=()) -> ALL topics.
    text = _read(domain_claude_md)
    embedded_block = _render_embedded_thinking_block(scope_prefixes=(), g=g)
    text = _substitute_embedded_thinking_block(text, embedded_block)
    _write(domain_claude_md, text)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.demo:
        ...
        _update_claude_md_domain_crystal()
        _update_claude_md_recently_rejected(g)
