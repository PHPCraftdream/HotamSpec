...
BANNER = (
    ...
    "hand. Edits: docstrings/content -> python tools/gen_spec.py -->"
)
...
def build_live_state(g: TensionGraph) -> str:
    ...
    approx_size = (
        len(_memo_block("repo_map", lambda: _scan_repo_map(graph=g)))
        + len(_memo_block("thinking_index", _render_thinking_index_block))
...
def render_mind_content(g: TensionGraph) -> str:
    ...
    parts = [
        ...
        _wrap(
            ...
            _THINKING_INDEX_END,
            _memo_block("thinking_index", _render_thinking_index_block),
        ),
        _wrap(
            _REPO_MAP_BEGIN,
            _REPO_MAP_END,
            _memo_block("repo_map", lambda: _scan_repo_map(graph=g)),
        ),
    ]
    return "\n".join(parts)
