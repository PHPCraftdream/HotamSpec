...
def _render_embedded_thinking_block() -> str:
    """Render the EMBEDDED-THINKING block: full content of every §Topic doc.

    Sourced in-memory from build_shared_thinking_docs() (not re-read from disk)
    so ordering within a single gen_spec.py run cannot race the on-disk writes.
    Deterministic: sorted by topic slug.
    """
    lines: list[str] = [
        "<!-- (generated by tools/gen_spec.py — do not hand-edit) -->",
        "",
        "### Embedded thinking (full methodology content)",
        "",
        "_Full content of every `spec/docs/thinking/*.md` file, embedded so the "
        "sole operator boots from substrate without a second file read._",
        "",
    ]
    docs = build_shared_thinking_docs()
    if not docs:
        lines.append("_(no thinking docs generated yet)_")
        return "\n".join(lines).rstrip()
...
def _update_claude_md_embedded_thinking() -> None:
    """Rewrite (or insert) the EMBEDDED-THINKING sentinel block in root CLAUDE.md.
    ...
    if not CLAUDE_MD.exists():
        return
    text = _read(CLAUDE_MD)
    new_block = _render_embedded_thinking_block()
...
def _render_embedded_tools_block() -> str:
    """Render the EMBEDDED-TOOLS block: full content of every tool doc.

    Sourced in-memory from build_shared_tool_docs() (not re-read from disk).
    All tools embedded, unfiltered — single operator, no scope split yet.
    Deterministic: sorted by basename.
    """
    lines: list[str] = [
        "<!-- (generated by tools/gen_spec.py — do not hand-edit) -->",
        "",
        "### Embedded tools (full tool doc content)",
        "",
        "_Full content of every `spec/docs/tools/*.md` file, embedded so the "
        "sole operator boots from substrate without a second file read. "
        "Unfiltered — all tools are in scope for the sole operator._",
        "",
    ]
    docs = build_shared_tool_docs()
    if not docs:
        lines.append("_(no tool docs generated yet)_")
        return "\n".join(lines).rstrip()
...
def _update_claude_md_embedded_tools() -> None:
    """Rewrite (or insert) the EMBEDDED-TOOLS sentinel block in root CLAUDE.md.

    Position: AFTER EMBEDDED-THINKING:END. Idempotent.
    """
    if not CLAUDE_MD.exists():
        return
    text = _read(CLAUDE_MD)
    new_block = _render_embedded_tools_block()

    if _EMBEDDED_TOOLS_BEGIN in text and _EMBEDDED_TOOLS_END in text:
        begin_pos = text.find(_EMBEDDED_TOOLS_BEGIN)
        end_pos = text.find(_EMBEDDED_TOOLS_END)
        before = text[: begin_pos + len(_EMBEDDED_TOOLS_BEGIN)]
        after = text[end_pos:]
        new_text = before + "\n" + new_block + "\n" + after
    else:
        pos = text.find(_EMBEDDED_THINKING_END)
        if pos != -1:
            insert_at = pos + len(_EMBEDDED_THINKING_END)
            new_text = (
                text[:insert_at]
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.demo:
        # P22.C consolidation: exactly ONE CLAUDE.md (repo root). While only one
        # domain exists and zero sub-agents are actively spawned, all blocks
        # render directly into root's own sentinels — no more domain-active vs
        # legacy-mode branching, no more DOMAIN-CRYSTAL indirection through a
        # separate domains/<name>/CLAUDE.md file.
        ...
        _update_claude_md_concept_map()
        _update_claude_md_thinking_index()
        _update_claude_md_embedded_thinking()
        _update_claude_md_embedded_tools()
