...
def _render_embedded_thinking_block() -> str:
    """Render the EMBEDDED-THINKING block: full body of spec/docs/thinking/*.md.
    ...
    thinking docs have been (re)written in this run.
    """
    ...
    for slug in sorted(docs):
        lines.append(docs[slug].rstrip())
        lines.append("")
    return "\n".join(lines).rstrip()


def _render_embedded_tools_block() -> str:
    """Render the EMBEDDED-TOOLS block: full body of spec/docs/tools/*.md.

    Content is built directly from tool docstrings + --help output
    (build_shared_tool_docs), not read back from disk, for the same
    determinism reason as _render_embedded_thinking_block().
    """
    ...
    for basename in sorted(docs):
        lines.append(docs[basename].rstrip())
        lines.append("")
    return "\n".join(lines).rstrip()
...
def render_mind_content(g: TensionGraph) -> str:
    ...
    Order: REPO-MAP, THINKING-INDEX, EMBEDDED-THINKING, EMBEDDED-TOOLS.
    """
    parts = [
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.demo:
        active = _active_domain()
        # Domain's own CLAUDE.md (if any) still gets all 5 blocks -- sub-agents
        # and per-domain tooling read that file directly.
        ...
        # RECENTLY-REJECTED), replacing the old ~10-sentinel surgical splice.
