    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Name) and node.func.id == "Violation":
                if len(node.args) >= 3 and isinstance(node.args[2], (ast.Constant, ast.JoinedStr)):
...
def _audit_invariant(func: object) -> tuple[str, str]:
    ...
    if len(entity_types) > 1:
        reasons.append(f"loops over {len(entity_types)} entity types: {', '.join(entity_types)}")
